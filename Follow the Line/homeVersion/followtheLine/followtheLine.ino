...
int recoverSpeed = carSpeed / 2;  // Forward speed - declared normally, as we may want to modify speed. 0 - 255.
int memory =0;
...
void loop() {
  ...
  */
  ...
  if ((digitalRead(L_S)) == 0 && (digitalRead(M_S)) == 1 && (digitalRead(R_S)) == 0) {
    ...
    curve_right();
    memory = 6;
  } else if ((digitalRead(L_S)) == 0 && (digitalRead(M_S)) == 0 && (digitalRead(R_S)) == 0){
    if (memory == 1){
      snap();
    } else if (memory == 2){
      sharp_right();
...
void snap() {
  ...
  for(int i = 0; i < 10; i++) {
    ...
    if(digitalRead(L_S) == 1 || digitalRead(M_S) == 1 || digitalRead(R_S) == 1) {
      // If line found, curve on the spot to align properly
        align_on_line_left();
      return;
    }
  }
  ...
  for(int i = 0; i < 10; i++) {
    analogWrite(ENA, recoverSpeed);
    digitalWrite(PhA, HIGH);
    analogWrite(ENB, recoverSpeed);
  ...
  // If still no line, stop briefly
  stop_car();
}

// Align functions: spin in place until left+middle or right+middle is detected
void align_on_line_left() {
  while(!(digitalRead(L_S)==1 && digitalRead(M_S)==1)) {
    analogWrite(ENA, recoverSpeed);
...
void align_on_line_right() {
  while(!(digitalRead(R_S)==1 && digitalRead(M_S)==1)) {
    analogWrite(ENA, recoverSpeed);
    digitalWrite(PhA, HIGH);
    analogWrite(ENB, recoverSpeed);
    digitalWrite(PhB, LOW);
...
void curve_left(){
  ...
  digitalWrite(PhA, HIGH);
  //RIGHT MOTOR
  analogWrite(ENB, carSpeed*0.8);
  digitalWrite(PhB, HIGH);
}
void curve_right(){
  //LEFT MOTOR
  analogWrite(ENA, carSpeed*0.8);
  digitalWrite(PhA, HIGH);
  //RIGHT MOTOR
